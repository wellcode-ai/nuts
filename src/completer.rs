...
impl NutsCompleter {
    pub fn new() -> Self {
        ...
        commands.insert(
            "security".to_string(),
            "Security analysis: security <URL> [OPTIONS]".to_string(),
        );

        // Flow Management
        commands.insert(
            "flow new".to_string(),
            "Create new flow: flow new <name>".to_string(),
        );
        commands.insert(
            "flow add".to_string(),
            "Add endpoint: flow add <name> <METHOD> <path>".to_string(),
        );
        commands.insert(
            "flow run".to_string(),
            "Run endpoint: flow run <name> <endpoint>".to_string(),
        );
        commands.insert(
            "flow docs".to_string(),
            "Generate docs: flow docs <name> [format]".to_string(),
        );
        commands.insert(
            "flow mock".to_string(),
            "Start mock server: flow mock <name> [port]".to_string(),
        );
        commands.insert("flow list".to_string(), "List all flows".to_string());
        commands.insert(
            "flow configure_mock_data".to_string(),
            "Configure mock data: flow configure_mock_data <name> <endpoint>".to_string(),
        );
        commands.insert(
            "flow story".to_string(),
            "Start AI-guided API workflow: flow story <name>".to_string(),
        );
        commands.insert(
            "flow s".to_string(),
            "Quick story mode alias: flow s <name>".to_string(),
        );
        commands.insert(
            "save".to_string(),
            "Save last request: save <flow> <name>".to_string(),
        );
