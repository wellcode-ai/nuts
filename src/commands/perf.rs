...
use crate::models::metrics::{Metrics, MetricsSummary, RequestMetric};
...
impl PerfCommand {
    ...
    pub async fn run(
        ...
        while start_time.elapsed() < duration {
            ...
            print!(
                "\r⚡ {} req ({} ok, {} ko) | {} req/s | lat: avg {}ms p95 {}ms | {}",
                ...
                if summary.error_rate > 0.0 {
                    style(format!("errors: {:.1}%", summary.error_rate * 100.0))
                        .red()
                        .bold()
                        .to_string()
                } else {
        ...
        let ok_requests =
            (final_summary.total_requests as f64 * (1.0 - final_summary.error_rate)) as usize;
        ...
        println!(
            "   • Total: {}",
            style(final_summary.total_requests).magenta().bold()
        );
        if final_summary.error_rate == 0.0 {
            ...
            println!(
                "   • OK: {} ({:.1}%)",
                style(ok_requests).green().bold(),
                style(format!("{:.1}", (1.0 - final_summary.error_rate) * 100.0))
                    .green()
                    .bold()
                    .to_string()
            );
            println!(
                "   • KO: {} ({:.1}%)",
                style(ko_requests).red().bold(),
                style(format!("{:.1}", final_summary.error_rate * 100.0))
                    .red()
                    .bold()
                    .to_string()
            );
        ...
        println!(
            ...
            style(format!(
                "{:.1}",
                final_summary.total_requests as f64 / duration.as_secs_f64()
            ))
            .yellow()
            .bold()
        );
        println!(
            "   • Peak: {} req/s",
            style(final_summary.peak_rps).magenta().bold()
        );
        ...
        println!(
            "\n{}  {}",
            style("⏱️").cyan(),
            style("Response Time Distribution").bold()
        );
        ...
        println!(
            "\n{}  {}",
            style("📈").cyan(),
            style("Response Time Details").bold()
        );
        println!(
            "   • Min: {}ms",
            style(
        ...
        println!(
            "   • Average: {}ms",
            style(final_summary.avg_latency.as_millis()).yellow().bold()
        );
        println!(
            "   • Median (p50): {}ms",
            style(final_summary.median_latency.as_millis())
                .yellow()
                .bold()
        );
        println!(
            "   • p95: {}ms",
            style(final_summary.p95_latency.as_millis()).yellow().bold()
        );
        println!(
            "   • p99: {}ms",
            style(final_summary.p99_latency.as_millis())
                .magenta()
                .bold()
        );
        println!(
        ...
        println!(
            "   • Std Dev: {}ms",
            style(format!("±{:.1}", final_summary.std_dev_latency)).dim()
        );
        ...
        match self
            .get_performance_analysis(&final_summary, duration)
            .await
        {
            Ok(analysis) => {
