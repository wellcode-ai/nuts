...
impl FixCommand {
    ...
    async fn diagnose_api(&self, url: &str) -> Result<ApiDiagnosis, Box<dyn std::error::Error>> {
        ...
        match call_command.execute_with_response(&["GET", url]).await {
            Ok(response) => {
                ...
                if response_time > 2000 {
                    diagnosis
                        .performance_issues
                        .push("Very slow response time".to_string());
                } else if response_time > 1000 {
                    diagnosis
                        .performance_issues
                        .push("Slow response time".to_string());
                }
                ...
                if response.is_empty() {
                    diagnosis
                        .response_issues
                        .push("Empty response body".to_string());
                }

                if response.contains("error") || response.contains("Error") {
                    diagnosis
                        .response_issues
                        .push("Response contains error messages".to_string());
                }
                ...
                if let Err(_) = serde_json::from_str::<Value>(&response) {
                    if !response.trim().starts_with('<') {
                        ...
                        diagnosis
                            .response_issues
                            .push("Invalid JSON response".to_string());
                    }
                }
            }
            Err(e) => {
        ...
        if !url.starts_with("https://") {
            diagnosis
                .security_issues
                .push("Not using HTTPS".to_string());
        }

        // Test common problematic endpoints
        for test_path in &["/admin", "/.env", "/debug", "/test"] {
            let test_url = format!("{}{}", url.trim_end_matches('/'), test_path);
            if let Ok(_) = call_command
                .execute_with_response(&["GET", &test_url])
                .await
            {
                diagnosis
                    .security_issues
                    .push(format!("Exposed sensitive endpoint: {}", test_path));
            }
        }
    ...
    ) -> Result<Vec<Fix>, Box<dyn std::error::Error>> {
        let api_key = self
            .config
            .anthropic_api_key
            .as_ref()
    ...
    async fn offer_automated_fixes(
        &self,
        url: &str,
        fixes: &[Fix],
        ...
        if !automated_fixes.is_empty() {
            ...
            println!(
                "🚀 Consider using 'security {}' for detailed security analysis.",
                url
            );
        } else {
