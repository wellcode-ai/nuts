...
impl AskCommand {
    ...
    pub async fn execute(&self, request: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if let Some(ContentBlock::Text { text }) = response.content.first() {
            ...
            if let Ok(ai_response) = serde_json::from_str::<Value>(text) {
                ...
                let follow_up = ai_response
                    .get("follow_up")
                    .and_then(|v| v.as_str())
                    .unwrap_or("What would you like to do next?");
    ...
    async fn execute_generate_data(
        &self,
        ai_response: &Value,
    ) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let data_type = ai_response
            .get("data_type")
            .and_then(|v| v.as_str())
            .unwrap_or("users");
        let count = ai_response
            .get("count")
            .and_then(|v| v.as_u64())
